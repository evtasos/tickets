...
RUN apt-get update && apt-get install -y dnsutils openssl && rm -rf /var/lib/apt/lists/*
...
CMD ["gunicorn", "-w", "4", "-b", "0.0.0.0:5555", "--certfile=cert.pem", "--keyfile=key.pem", "app:app"]
